...
if RPYC_LEADER_PORT == '':
    RPYC_LEADER_PORT = random_port(18000,19000, 10)
...
def get_worklist_node():
    global zkClient
    lwork={}
    if zkClient.exists(MONITOR_PARENT_PATH + "/worker/worker"):
        children = zkClient.get_children(MONITOR_PARENT_PATH + "/worker/worker")
        if len(children) == 0:
            #worker全部消亡
            ...
            for child in children:
                if not lwork.has_key(child) and child <> 'path':
                    ...
                    lwork[child]['path'] = get_zknode_str_value(zkClient, str(MONITOR_PARENT_PATH + "/worker/worker/path/" + child))
...
class RpycManagerService(rpyc.Service):
    ...
    def exposed_QueryWorks(self):
        ...
        lworkers['leader']['path'] = get_zknode_str_value(zkClient, str(MONITOR_PARENT_PATH + "/worker/leader/path"))
        #环境信息
        try:
            # 获取硬件信息
            import psutil
...
def taskdispacher_leader_func():
    ...
    try:
        # 更新zknode
        zkClient.ensure_path(MONITOR_PARENT_PATH + "/worker/leader")
        zkClient.set(MONITOR_PARENT_PATH + "/worker/leader", str(RPYC_LEADER_HOST) + '_' + str(RPYC_LEADER_PORT))
        zkClient.ensure_path(MONITOR_PARENT_PATH + "/worker/leader/path")
        zkClient.set(MONITOR_PARENT_PATH + "/worker/leader/path", sysdir.decode("GBK").encode("utf-8"))
...
if __name__ == '__main__':
    try:
        ...
        zkClient.start()
        if not zkClient.exists(MONITOR_PARENT_PATH + "/worker/electionpath"):
            # 确认路径，如果有必要则创建该路径
            zkClient.ensure_path(MONITOR_PARENT_PATH + "/worker/electionpath")
        vprint('waiting for the election!', None,logger, logging.INFO)
        # 阻塞线程，直到选举成功。并调用taskdispacher_leader_func
        election = zkClient.Election(MONITOR_PARENT_PATH + "/worker/electionpath")
